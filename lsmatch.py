...
class LabelSmoothingMatch(CTAReMixMatch):
    ...
    def model(self, batch, lr, wd, wu, confidence, uratio, ema=0.999, dbuf=128, **kwargs):
        ...
        # p_data_tf = tf.compat.v1.Print(p_data(), [p_data()], "p_data=", summarize=25)
        # p_model_tf = tf.compat.v1.Print(p_model(), [p_model()], "p_model=", summarize=25)
        ...
        # alpha = tf.compat.v1.Print(alpha, [alpha], "alpha=", summarize=25)
...
def main(argv):
    ...
    model = LabelSmoothingMatch(
        os.path.join(FLAGS.train_dir, dataset.name, LabelSmoothingMatch.cta_name()),
        dataset,
        lr=FLAGS.lr,
        wd=FLAGS.wd,
        arch=FLAGS.arch,
